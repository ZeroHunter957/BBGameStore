            ...
            if (filterType === 'reviews-made') {
                ...
                fetch("{{ route('profile.feedbacks') }}")
                    ...
                    .then(data => {
                        ...
                        // Loop through feedbacks and display them using Bootstrap styling
                        data.feedbacks.forEach(feedback => {
                            const feedbackElement = document.createElement('div');
                            feedbackElement.classList.add('card', 'mb-3');
                            feedbackElement.innerHTML = `
                                <div class="card-body">
                                    <h5 class="card-title">${feedback.game.title}</h5>
                                    <p class="card-text">${feedback.content}</p>
                                    <span class="badge bg-warning">${feedback.star} Stars</span>
                                    <div class="text-muted mt-2">Reviewed on ${new Date(feedback.created_at).toLocaleDateString()}</div>
                                    <!-- Game Details Link -->
                                    <a href="{{ url('gamedetails') }}/${feedback.game.id}" class="btn btn-primary mt-3">View Game Details</a>
                                </div>
                            `;
